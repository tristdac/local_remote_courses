...
require_once($CFG->dirroot."/theme/remui_child/classes/utility.php");
...
class local_remote_courses_external extends external_api {
    ...
    public static function get_courses_by_username($username) {
        ...
        // Get the courses.
        $view_pref = get_user_preferences('qcl_view_state','',$userid);
        // if ($view_pref) {
            if ($view_pref === 'starred') {
                $preference = 'Starred';
                $courses = \theme_remui_child\utility::get_starred_courses($userid);
            }
            if ( ($view_pref === 'recent') || ( ($view_pref === "starred" && empty($courses)) || ($view_pref === "current" && empty($courses) ) )  ){
                $preference = 'Recent';
                $courses = \theme_remui_child\utility::get_recent_accessed_courses($userid);
            } 
            if (!$view_pref || $view_pref === 'current') {
                $preference = 'Current';
                $finite = \theme_remui_child\utility::get_current_courses_finite($userid);
                $persistent = \theme_remui_child\utility::get_current_courses_persistent($userid);
                $courses = array_merge($finite,$persistent);
            } 
        // }
        ...
        $result[] = array(
                'id' => '1',
                'shortname' => $preference.' Courses',
                'fullname' => $preference.' Courses',
                'term' => '',
                'visible' => '0',
                'roles' => array(),
            );
